...
namespace FileExplorer
{
    public partial class Form1 : Form
    {
        ...
        private void btnFiltrele_Click(object sender, EventArgs e)
        {
            ...
            string[] SelectedSubString = seciliUzanti.Split('(');
            string myExtensionSub = SelectedSubString[0];
            DirectoryInfo dosyalarDizini = new DirectoryInfo(treeView1.SelectedNode.Tag.ToString());
            ...
            if (cmbFiltre.SelectedIndex > 0)
            {
                ...
                //{
                //    if (Path.GetExtension(itm.Text.ToLower()) != myExtensionSub)
                //        itm.Remove();
                ...
                {
                    if (Path.GetExtension(dosya.ToString().ToLower()) == myExtensionSub)
                    {

                        // item.Text = dosya.ToString();
                        var listViewItem = new ListViewItem(new[] {
                            dosya.ToString(),
                            dosya.Length.ToString(),
                            dosya.CreationTime.ToShortTimeString()
                        });
        ...
        void AltKlasorDoldur(TreeNode nd)
        {
            try
            {
                ...
                foreach (DirectoryInfo diAltKlasor in altKlasorler)
                {
        ...
        void DosyalariDoldur(string path)
        {
            ...
            FileInfo[] dosyalar = dosyalarDizini.GetFiles();
            Dictionary<string, int> dosyaSayisi = new Dictionary<string, int>();
            listView1.Items.Clear();
            ...
            foreach (FileInfo fiDosya in dosyalar)
            {
                ...
                //Aşağıdaki kısımda 
                if (dosyaSayisi.ContainsKey(uzanti))
                {
                    dosyaSayisi[uzanti]++;
                }
                else
                {
                    dosyaSayisi.Add(uzanti, 1);
                }
            ...
            foreach (var item in dosyaSayisi)
            {
                cmbFiltre.Items.Add(item.Key + "(" + (item.Value) + ")");
            }
        }
